...
def VAE(original_dim = (224,224,3), epsilon_std = 1.0, lr = 0.0001, is_sum = True, bottleneck_filter_size=(1,1), if_localconv = False): 
    ...
    if if_localconv:
        z_mean = LocallyConnected2D(512, bottleneck_filter_size, activation = 'linear', padding = 'valid', name = 'z_mean')(x)
        z_log_var = LocallyConnected2D(512,bottleneck_filter_size,activation='linear',padding = 'valid',name='z_log_var')(x)
    else:
        z_mean = Conv2D(512,bottleneck_filter_size, activation='linear',padding='same',name='z_mean')(x)
        z_log_var = Conv2D(512,bottleneck_filter_size, activation='linear',padding = 'same',name='z_log_var')(x)
