...
is_sum = True
b_size = 32
#saved_weights = 'weights/debug_01_lr0.0001_latentdim2048_nomeanfile_sumloss-best-06-5476.68.hdf5'
saved_weights = None
lr_rate = 0.0001  
...
prefix = 'debug_01_lr'+ str(lr_rate) + '_latentdim' + str(latent_dim)
...
# generators
if mean_img_file is not None:
    train_datagen = FaceAugDataGen(mode = 'training', batch_size=b_size ,im_shape = (224,224), source = data_path, mean_file = mean_img_file, latent_dim = latent_dim )
    val_datagen = FaceAugDataGen(mode = 'validation', batch_size=b_size ,im_shape = (224,224), source = data_path, mean_file = mean_img_file ,latent_dim = latent_dim)
else:
    train_datagen = FaceAugDataGen(mode = 'training', batch_size=b_size ,im_shape = (224,224), source = data_path, latent_dim = latent_dim)
    val_datagen = FaceAugDataGen(mode = 'validation', batch_size=b_size ,im_shape = (224,224), source = data_path, latent_dim = latent_dim)
...
#H = multi_model.fit_generator(generator = train_datagen, steps_per_epoch = 1000, epochs = 600, validation_data = (valid_X, valid_Y),  callbacks =mycallbacks)
#H = model.fit_generator(generator = train_datagen, steps_per_epoch = train_steps_per_epoch, epochs = 15, validation_data = (valid_X, [valid_X1_1,valid_X1_2]), callbacks =mycallbacks )
#multi_model.evaluate(x = valid_X, y = valid_Y)
train_samples = 300
valid_samples = 20
latent_dim = 10
train_x = valid_X[0:train_samples]
valid_x = valid_X[train_samples:train_samples + valid_samples]
H = model.fit(train_x,[train_x,np.zeros((train_samples,1))], batch_size = 5,validation_data = (valid_x,[valid_x,np.zeros((valid_samples,1))] ), epochs = 15)

predict_X = model.predict(valid_X)[0]
predict_X = predict_X * 255.
predict_X = predict_X + 128
#predict_X = np.clip(predict_X,0,255)
...
for in_x,x in zip(valid_X,predict_X):  
    cv2.imwrite('in_'+str(i)+'.jpg', in_x.transpose(1,2,0))
